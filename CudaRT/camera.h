...
public:
	...
	__device__ color render(curandState* state, const hittable& world, const int i, const int j) {
		//initialize();
		...
		{
			ray r = get_ray(state, i, j);
			pixel_color += ray_color(r, max_depth, world, state);
		}
...
private:
	...
	__device__ ray get_ray(curandState* state, int i, int j) const {
	...
	__device__ vec3 sample_disk(curandState* state, double radius) const {
	...
	__device__ color ray_color(const ray& r, int depth, const hittable& world, curandState* state) const {
		// if we've exceeded the ray bounce limit, no more light in gathered.

		//ray current_ray = r;
		//color final_color = color(1.0, 1.0, 1.0);
